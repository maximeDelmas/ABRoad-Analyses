...
# Task 1: Fungus [X] produces chemical [X]
...
## Load results
```{r}
data_JS_divergences <- read_tsv("data/JS_divergence_table_complete_task1_5000.tsv")
...
# laod top 20
top_20 <- read_tsv("data/top_k_table_task1.tsv")
...
top1_cor <- read_tsv("data/top_1_cor_task1.tsv")
...
La taille d'effet maximale observé est autour de 3%, ce qui est très faible.
...
ggplot(prop_data_computation, aes(model, factor(Template), fill = vs_Pos)) + 
  geom_tile(color = "white",
            lwd = 1.5,
            linetype = 1) +
  geom_text(aes(label = round(vs_Pos, 2)), color = "white", size = 6) +
  scale_fill_gradient2(low = "#075AFF",
                       mid = "#FFFFCC",
                       high = "#FF0000",
                       limits = c(0,1)) +
...
## Top 20 tokens
WARNING: COUNTS ARE (EXPECTED COUNTS) SUM OF PROBA
