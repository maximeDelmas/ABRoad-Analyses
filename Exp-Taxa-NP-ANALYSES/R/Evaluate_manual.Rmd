---
title: "Evaluate predictions - Manual"
...
model_ref <- c("P703-PubMedBERT*", "rP703-PubMedBERT-full*")
...
model_res_masked <- c("output/np/manual/PubMedBERT/manual2/confidence/none/MASKED/P703.json",
                      "output/np/manual/PubMedBERT-full/manual1/order/confidence/MASKED/rP703.json")
...
- Le modèle sur la relation P703 semble potentiellement soumis au prompt-bias. Pour **toutes** les expériences (1934 sujets du dataset *test*), au moins un des objets prédits par le modèle est commun à ceux prédits par ce même modèle lorsque le sujet est maské.

- Pour la relation rP703 à l'inverse, le modèle semble peu affecter par ce bias. Pour seulement 12/500 des expériences, le modèle a prédit un objet commun à ceux prédit lorsque le sujet est maské.

Nombre total de prédictions concernées.
```{r}
...
* Pour la propriétée P703:
  * On sait que **pour toutes** les expériences, il y a au moins une prédiction dans le top50 qui aurait put être faite par le modèle alors que le sujet avait été maské.
  * Lorsque l'on regarde la distribution des rangs, on constate qu'il s'agit de top prédictions dams les deux configuration, signifiant que ces objets sont parmis les plus probables prédits par le modèle, que le sujet soit maské ou non.
  * Par exemple, lorsqu'une prédiction implique un objet qui aurait aussi pu être prédit sans que le sujet soit spécifié, plus de 25\% du temps, elle est classé top1 des prédictions dans les 2 configuration.
  * La corrélation de SPEARMAN nous indique que les rangs sont corrélé de manière modéré. 
  
  * Le plus flagrant est certainement sur la seconde figure. Cette histogramme montre la répartition du type de prédiction par rang, avec "exclusive" si cette prédiction n'est jamais prédite par le modèle lorsque le sujet est maské, "top1" lorsque cette prédiction est la top1 prédite par le modèle lorsque le sujet est maské, top [2-5[ lorsqu'elle est dans le top 2 à 5, etc ...
  
  * On constate donc clairement que pour les top prédictions:
    * le top1 est quasi-systématiquement commun avec la prédiction lorsque le sujet est maské -> gros promt-bias.
    * jusqu'au top10 on a envirion 25% des prédictions qui sont communes à ce qu'aurait pu prédire le modèle sans sujet !
    * Notons que l'on retrouve rarement des prédictions faite par le modèle sans sujet > au top10

Correlation de SPEARMAN (sur les rangs donc)
```{r}
...
bias_from_training_dataset("data/np/triples_processed/P703/train.jsonl", 1546, big.df, "P703-PubMedBERT*")
...
data_for_plot %>% filter(is.correct) %>% group_by(ref, preds.rank, .drop=F) %>% summarise(n = n()) %>% mutate(prop = (n / sum(n)) * 100 ) %>% 
  ...
  theme_classic() +
  facet_grid(. ~ ref) +
  theme(axis.text = element_text(size = 10), axis.text.y = element_text(size = 20), text = element_text(size = 25))
```
...
Un exemple ambigue qui peut se présenter, est une prédiction pour laquelle le sujet est annoté avec un LCP fort, mais pour autant, l’objet prédit par le modèle ne présente pas de préfixe commun. 
...
See full table with frequencies and LCP:

```{r}

correcte_preds_and_freq %>% mutate(prop = str_split(ref, "-", simplify = T)[, 1]) %>% 
  left_join(LCP_for_join, by = c("prop", "subject.id")) %>% view()
