...
```{r}
t1 <- get_longer_data("../ABRoad_Masking/data/JS_divergence_table_complete_task1_5000.tsv")
t1$Task <- "task1"
t2 <- get_longer_data("../ABRoad_Masking/data/JS_divergence_table_complete_task2_5000.tsv")
t2$Task <- "task2"

t <- rbind(t1, t2)

# On Ajoute aussi +1 au template car j'aime pas avoir les indinces à 0 ...
...
C'est quoi le Cliff-delta: C'est la différence entre la proba que X > Y et Y > X. Dans notre cas, on peut donc l'interpréter comme la différences des probabilités d'observer une divergence de Jensen-Shanon  supérieure en comparant un statement postif contre un statement négatif, plutèot qu'en comparant deux statements positifs ou négatifs.

Lorsque l'on compare 2 variable $X$ et $Y$:
...
t1_top1_cor <- read_tsv("../ABRoad_Masking/data/top_1_cor_task1.tsv")
t1_top1_cor$Task <- "task1"
t2_top1_cor <- read_tsv("../ABRoad_Masking/data/top_1_cor_task2.tsv")
